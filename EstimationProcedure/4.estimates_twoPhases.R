...
#        areas_2015_2024.csv
#        /3.estimates2022proTwoPhasesLight
# Output: /4.TwoPhaseEstimates2022
...
dire <- getwd()
direnew1 <- paste(dire, "/4.TwoPhaseEstimates2022", sep = "")
# if (dir.exists(direnew1))
#   unlink(direnew1,recursive=TRUE)
if (!dir.exists(direnew1))
  dir.create(direnew1)
# 
direnew2 <- paste(dire, "/4.TwoPhaseWeights2022", sep = "")
# if (dir.exists(direnew2))
#   unlink(direnew2,recursive=TRUE)
if (!dir.exists(direnew2))
  dir.create(direnew2)
# 
direnew3 <- paste(dire, "/4.TwoPhaseSamples", sep = "")
...
for (i in c(1:length(paesi))) {
  ...
  # 2nd PHASE
  #---------------------------------------------------------------
  source("4a.estimates_twoPhases_secondPhase.R")
